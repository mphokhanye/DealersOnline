            ...
            return (
              <div key={car.id} className="bg-card border-[1.5px] border-sand rounded-2xl overflow-hidden mb-2.5">
                ...
                <div className="p-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    <button onClick={() => openModal("fuel", car)} className="bg-warning-bg border-[1.5px] border-warning/30 text-warning text-xs font-semibold px-2 py-2.5 rounded-lg cursor-pointer flex items-center justify-center gap-1.5 hover:bg-warning/20 transition-colors">
                      <Fuel size={13} /> Fuel cost
                    </button>
                    <button onClick={() => openModal("reduce", car)} className="bg-success-bg border-[1.5px] border-success/30 text-success text-xs font-semibold px-2 py-2.5 rounded-lg cursor-pointer flex items-center justify-center gap-1.5 hover:bg-success/20 transition-colors">
                      <Tag size={13} /> Reduce price
                    </button>
                    <button
                      onClick={() => toggleSave(car.id)}
                      className={`border-[1.5px] text-xs font-semibold px-2 py-2.5 rounded-lg cursor-pointer flex items-center justify-center gap-1.5 transition-colors ${
                        isSaved ? "bg-terra text-primary-foreground border-terra" : "bg-terra/10 text-terra border-terra/30 hover:bg-terra/20"
                      }`}
                    >
                      <Heart size={13} className={isSaved ? "fill-current" : ""} /> {isSaved ? "Saved" : "Save car"}
                    </button>
                  </div>
